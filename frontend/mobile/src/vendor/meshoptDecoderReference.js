...
MeshoptDecoder.decodeVertexBuffer = (target, elementCount, byteStride, source, filter) => {
  ...
  const channels =
    version === 0 ? null : source.slice(tailDataOffs + byteStride, tailDataOffs + tailSize);
  ...
  for (let dstElemBase = 0; dstElemBase < elementCount; dstElemBase += maxBlockElements) {
    ...
    for (let byte = 0; byte < byteStride; byte++) {
      ...
      const controlMode =
        version === 0
          ? 0
          : (source[controlBitsOffs + (byte >>> 2)] >>> ((byte & 0x03) << 1)) & 0x03;
  ...
  if (filter === 'OCTAHEDRAL') {
    ...
    const dst = new (byteStride === 4 ? Int8Array : Int16Array)(
      target.buffer,
      target.byteOffset,
      elementCount * 4
    );
    ...
    const data = new (byteStride === 4 ? Uint8Array : Uint16Array)(
      target.buffer,
      target.byteOffset,
      elementCount * 4
    );
    const dataSigned = new (byteStride === 4 ? Int8Array : Int16Array)(
      target.buffer,
      target.byteOffset,
      elementCount * 4
    );
