...
interface TestAsset {
  key: 'skin' | 'nervous' | 'reproductive' | 'cardiovascular';
  label: string;
  url: string;
}

const TEST_ASSETS: TestAsset[] = [
  { key: 'skin', label: 'Skin', url: '/models-dev/female-skin-meshopt.glb' },
  { key: 'nervous', label: 'Nervous System', url: '/models-dev/female-nervous-meshopt.glb' },
  {
    key: 'reproductive',
    label: 'Reproductive',
    url: '/models-dev/female-reproductive-meshopt.glb',
  },
  {
    key: 'cardiovascular',
    label: 'Cardiovascular',
    url: '/models-dev/female-cardiovascular-meshopt.glb',
  },
];

type LoadState = 'idle' | 'loading' | 'loaded' | 'error';

export default function FemaleTestPage(): JSX.Element {
  ...
  const selectAsset = useCallback((asset: TestAsset) => {
    ...
    setLoadDurationMs(0);
    setState('loading');
    window.performance.mark(`anatomiax-female-load-start-${asset.key}`);
    console.info(
      `[AnatomiaX female test] requesting ${asset.label} (${asset.url}) — THREE r${THREE.REVISION}`
    );
  }, []);
  ...
  const handleLoaded = useCallback((modelInfo: OptimizedModelInfo) => {
    ...
    setLoadDurationMs(() => {
      try {
        const keys = ['skin', 'nervous', 'reproductive', 'cardiovascular'];
        const marks = keys
          .map(k => window.performance.getEntriesByName(`anatomiax-female-load-start-${k}`))
          .flat();
        const last = marks.pop() as PerformanceMark | undefined;
        return last ? Math.round(window.performance.now() - last.startTime) : 0;
      } catch {
        return 0;
      }
    });
    console.info('[AnatomiaX female test] loaded:', modelInfo);
    setState('loaded');
  }, []);
  ...
  const handleError = useCallback((message: string) => {
    ...
    setState('error');
    console.error('[AnatomiaX female test]', message);
  }, []);
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100">
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        ...
        <h1 className="mt-2 text-2xl font-bold tracking-tight sm:text-3xl">Female asset test</h1>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">
          Female anatomy browser test — verifies Meshopt-optimized female GLBs load via the same
          local decoder. Temporary copies from{' '}
